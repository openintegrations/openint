...
export function AddConnection({initialData}: {initialData?: unknown}) {
  ...
  const res = useSuspenseQuery(trpc.listConnectorConfigs.queryOptions({}))
...
export function ConnectionList({initialData}: {initialData?: unknown}) {
  // Simulate a random delay between 0-2 seconds
  // await new Promise((resolve) => setTimeout(resolve, Math.random() * 2000))
  ...
  const trpc = useTRPC() // use `import { trpc } from './utils/trpc'` if you're using the singleton pattern
  const res = useSuspenseQuery(trpc.listConnections.queryOptions({}))
