import {
  generateConnectorServer,
  generateConnectorServerFromSchemas,
} from '../_defaults'
...
connectorDef.connector_name
def.name

function test<T extends JsonConnectorDef>(def: T) {
  return {
    // @openint-bot why is this cast necessary? ask ai
    name: def.connector_name as T['connector_name'],
  }
}
const server = generateConnectorServerFromSchemas({
  schemas: def.schemas,
  overrides: {
