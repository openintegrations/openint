...
export const ConnectorTableCell = ({
  ...
  <div className="flex items-center gap-2">
    {connector.logo_url && (
      // TODO(@snrondina): Use next/image component instead of default html img tag
      // TODO(@openint-bot): Add a linter rule to enforce this
      <img
    ...
    {(showStage || showPlatforms) && (
      <div className="ml-2 flex gap-1">
        {showStage && connector.stage && (
          <Badge
            variant="outline"
            className={cn(
              connector.stage === 'ga' && 'bg-green-50 text-green-700',
              connector.stage === 'beta' && 'bg-blue-50 text-blue-700',
              connector.stage === 'alpha' && 'bg-pink-50 text-pink-700',
            )}>
            {connector.stage}
          </Badge>
        )}
        {showPlatforms &&
          connector.platforms?.map((platform: string, i: number) => (
