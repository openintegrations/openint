---
openapi: https://raw.githubusercontent.com/openintegrations/openSDKs/refs/heads/main/sdks/sdk-merge/merge_hris.oas.json
---
...
| **GET** | /bank-info | Returns a list of `BankInfo` objects.
| **GET** | /bank-info/{id} | Returns a `BankInfo` object with the given `id`.
| **GET** | /benefits | Returns a list of `Benefit` objects.
| **GET** | /benefits/{id} | Returns a `Benefit` object with the given `id`.
| **GET** | /companies | Returns a list of `Company` objects.
| **GET** | /companies/{id} | Returns a `Company` object with the given `id`.
...
| **GET** | /dependents | Returns a list of `Dependent` objects.
| **GET** | /dependents/{id} | Returns a `Dependent` object with the given `id`.
| **GET** | /employee-payroll-runs | Returns a list of `EmployeePayrollRun` objects.
| **GET** | /employee-payroll-runs/{id} | Returns an `EmployeePayrollRun` object with the given `id`.
| **GET** | /employees | Returns a list of `Employee` objects.
| **POST** | /employees | Creates an `Employee` object with the given values.
| **GET** | /employees/{id} | Returns an `Employee` object with the given `id`.
| **POST** | /employees/ignore/{model_id} | Ignores a specific row based on the `model_id` in the url. These records will have their properties set to null, and will not be updated in future syncs. The "reason" and "message" fields in the request body will be stored for audit purposes.
| **GET** | /employees/meta/post | Returns metadata for `Employee` POSTs.
| **GET** | /employer-benefits | Returns a list of `EmployerBenefit` objects.
| **GET** | /employer-benefits/{id} | Returns an `EmployerBenefit` object with the given `id`.
| **GET** | /employments | Returns a list of `Employment` objects.
| **GET** | /employments/{id} | Returns an `Employment` object with the given `id`.
...
| **GET** | /locations | Returns a list of `Location` objects.
| **GET** | /locations/{id} | Returns a `Location` object with the given `id`.
| **POST** | /passthrough | Pull data from an endpoint not currently supported by Merge.
| **GET** | /pay-groups | Returns a list of `PayGroup` objects.
| **GET** | /pay-groups/{id} | Returns a `PayGroup` object with the given `id`.
| **GET** | /payroll-runs | Returns a list of `PayrollRun` objects.
| **GET** | /payroll-runs/{id} | Returns a `PayrollRun` object with the given `id`.
...
| **GET** | /time-off | Returns a list of `TimeOff` objects.
| **POST** | /time-off | Creates a `TimeOff` object with the given values.
| **GET** | /time-off-balances | Returns a list of `TimeOffBalance` objects.
| **GET** | /time-off-balances/{id} | Returns a `TimeOffBalance` object with the given `id`.
| **GET** | /time-off/{id} | Returns a `TimeOff` object with the given `id`.
| **GET** | /time-off/meta/post | Returns metadata for `TimeOff` POSTs.
| **GET** | /timesheet-entries | Returns a list of `TimesheetEntry` objects.
| **POST** | /timesheet-entries | Creates a `TimesheetEntry` object with the given values.
| **GET** | /timesheet-entries/{id} | Returns a `TimesheetEntry` object with the given `id`.
| **GET** | /timesheet-entries/meta/post | Returns metadata for `TimesheetEntry` POSTs.
